...
const about = {
  title: "About me",
  description: "Passionate MERN Stack developer, Specializing in React JS, Next Js and Typescript.",
  info: [
...
const experience = {
  title: "My experience",
  description: "Passionate Frontend developer, Specializing in React JS, Next Js and Typescript.",
  items: [
    {
      ...
      <div className='flex flex-col gap-8'>
        {button.map((item,index) =>{
          return <button key={index} className={`${
            activeSection === `${item.id}` && "text-black bg-[#00FF99]  font-bold shadow-sm opacity-100 "
            } inline-flex items-center justify-center whitespace-nowrap text-white rounded-lg p-3 font-medium transition-all  opacity-50 hover:bg-[#00FF99] `} 
            onClick={() => setActiveSection(`${item.id}`)}>{item.name}</button>
        })}
      </div>
            ...
            <div className='flec flex-col gap-[30px] text-center xl:text-left'>
              ...
              <ul className='grid grid-cols-2 gap-4 sm:grid-cols-3 md:grid-cols-4 xl:gap-[30px]'>
                {skills.skillList.map((skill, index) => (
                  <li key={index} className='w-full h-[150px] bg-[#232329] rounded-xl flex justify-center items-center '>
                    <span className='text-6xl hover:text-[#00FF99]'>{skill.icon}</span><span className='capitalize'>{skill.name}</span>
                  </li>
                ))}
              </ul>
            </div>
